...
import { Search, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Truck } from "lucide-react";
...
export default function OdooVendors() {
  ...
  const { data, isLoading, error, isFetching } = useQuery<{ records: OdooPartner[]; total: number }>({
    queryKey: ["/api/odoo/partners", "supplier", debouncedSearch, offset, pageSize],
    queryFn: async () => {
      const res = await fetch(`/api/odoo/partners?${queryParams.toString()}`);
      if (!res.ok) throw new Error("Failed to fetch Odoo vendors");
      return res.json();
    },
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-6 text-center text-destructive" data-testid="text-error">
              Failed to load vendors from Odoo.
            </div>
          ) : !data?.records.length ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {data.records.map((p) => (
                    <TableRow key={p.id} data-testid={`row-vendor-${p.id}`}>
